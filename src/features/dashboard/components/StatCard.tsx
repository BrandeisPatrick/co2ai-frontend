    ...
    <div className="glass-card-hover rounded-xl p-3 md:p-5 cursor-default">
      <div className="flex items-start justify-between mb-3 md:mb-4">
        <div className="flex items-center gap-2 md:gap-3">
          <div className="p-2 md:p-3 bg-gradient-to-br from-emerald-500/20 to-emerald-600/20 rounded-lg border border-emerald-500/30">
            <Icon size={16} className="md:w-5 md:h-5 text-emerald-600" />
          </div>
        </div>
        {trend && (
      ...
      <div className="space-y-1">
        <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide md:tracking-wider">{title}</h3>
        <div className="flex items-baseline gap-1 md:gap-2">
          <span className="text-xl md:text-3xl font-bold text-gray-900">
            {value}
          </span>
          {unit && <span className="text-xs md:text-sm font-medium text-gray-400">{unit}</span>}
        </div>
        {trend && (
